...
export default function PipelineColumn({
  ...
  return (
    ...
    >
      ...
      {title === "Candidature" && totalReceived > 0 && (
        <div className="mb-2 text-center font-body text-xs text-text-muted">
          {count} affiché{count > 1 ? "s" : ""} sur {totalReceived} candidature
          {totalReceived > 1 ? "s" : ""} totale{totalReceived > 1 ? "s" : ""}
        </div>
      )}
      ...
      >
        {candidates.length > 0 ? (
          candidates.map(function (candidate) {
            ...
            var isTop = topCandidateIds.indexOf(candidateId) !== -1;
